...
class TrainConfig:
    ...
    # —— 两级评估（2026-09-18，长跑专用；默认全关 = 旧行为逐位不变）——
    # 动机：1M 步 + 8000 步一评 = 125+ 个评估点，若每点都是 40 局/对，评估吃掉
    # 60%+ 墙钟。两级方案：小评估维持曲线分辨率（每 steps_per_eval 步、n_eval_games
    # 局/对），大评估只在稀疏锚点（每 big_eval_every 步）用更大局数压低噪声。
    # 两个网格取**并集**：落在大评估网格但不在小网格的点会被**插入**（例：8000 与
    # 100000 不整除 ⇒ 100k/200k/…/900k 共 9 个额外大评估点）；同一格点只评一次、
    # 按大预算（不重复花算力）。
    big_eval_every: int = 0       # 大评估间隔（步）；0 = 关闭（旧行为：无大评估）
    n_eval_games_big: int = 0     # 大评估每对局数；0 = 回退到 n_eval_games（等价于只改间隔）
    eval_big_at_start: bool = True  # 起始点（step 0）用大评估预算：先拿一个低噪声基线点
    ...
    # 2026-09-17 用户拍板：**默认关闭**僵局早停。
    # 理由（用户原话）：「50 秒内未打掉塔不该被作为一个正确的减少训练时间的行为，
    # 为了那些对局，我们接受这些时间损失，更别提我们的平局跟失败都扣 10 分了」
    # ⇒ 零塔损的僵局不再在 50s 被判平截断，改为**打满常规时间/加时**（180s，皇冠平则到 300s）
    #   由引擎按"三塔血量合计"裁决。旧行为可复现：`--train-stall-stop`。
    # ⚠️ 已知代价：零塔损局（实测约占评估局 11%）从 100 帧变 600 帧 ⇒ 同 20k 步对应的
    #   **局数**从 ~55~80 降到 ~35~50（每步成本不变，变的是每局长度）；评估墙钟同步上升。
    train_stall_stop: bool = False
