        ...
        while open_heap:
            ...
            for neighbor in get_neighboring_points(current[0], current[1]):
                ...
                if tile_char == 'W':
                    ...
                    # 2026-09-19 用户拍板：恢复上游口径 `.` = 8（车道走廊 `1`/`2` 仍为 5）。
                    # 2026-09-10 曾把 `.` 改成与走廊同价 5（理由「代价相同 ⇒ 保持横向车道」），
                    # 实测后果：全图同价 ⇒ 「提前横移」与「到河再横移」**代价严格相等**
                    # （3 个斜步放哪一段都是 3×70+26×50），A* 只按 tie-break 出结果，
                    # 我方单位沿 x=6.00 一条垂直线走到 y≈13.4 才拐，上游则全程逐步收拢。
                    # 恢复 8 后 `1`/`2`（通往桥口的正对走廊）比走廊外便宜 3 ⇒ 越早进入越省
                    # ⇒ 横向收拢成为本征最优。证据与预注册：docs/o3_lateral_convergence_fix_2026-09-19.md
                    tile_cost = 8
                else:
                    tile_cost = 5
                if nx != px and ny != py:
