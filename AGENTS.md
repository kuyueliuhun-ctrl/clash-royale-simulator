...
| **★ `et_solo100k` 两臂干预长跑记录**（进行中；含**三次发射** + 三处测量侧更正 §11.13.6/7/9 + `eval@0` 三层读数） | [`docs/et_solo100k_2026-09-18.md`](docs/et_solo100k_2026-09-18.md) |
...
3. **★ 第八轮：用户直接要求的 `economy_et` 100k 干预长跑（两臂，进行中）**
   ...
   - 仪器：`scripts/judge_critic_inertia.py` 新增 `--baseline within`（本 run 前 100 诊断点中位 ± 3×MAD，
     **原始 MAD** 不乘 1.4826；【R4】脚本复算；`--selftest` 4/4 PASS）；
     一键判读器 `scripts/et_solo100k_readout.py`（§11.13.2 四层 + 按批次配对 A−B，缺数据大声失败）；
     判读文档必写条目见预注册 **§11.13.8**。
