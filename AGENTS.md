...
| **★ 分析：「骰子决定出不出牌」+「模型可选是否用骰子」（用户提问，**只分析不实现**）**：骰子作用域**只有 ~10% 帧**（其余 88~91% 被掩码强制）、**覆盖率 0.5~1.8%→~5%**；**i.i.d. 骰子给不了"攒费"需要的时域**（需连续 10~25 帧"有牌却不出"，i.i.d. 只有 `0.5^K`）⇒ **必须带时长的 option**；可学元门大概率**退化解**，且骰子帧对 slot_head 的 play/pass **梯度恒为 0**（只能当数据生成器）；**本卡组攒费通道是死代码**（`hold_mask ≡ 0`：`_setup_wait` 要 `SINK_TANK_CARDS`、`_save_ace` 要 `ACE_CARDS`，solo 卡组两张都不沾，独立吻合于"Δlogit 恒 −0.8000"）；**最大工程坑 = PPO 重放一致性**（骰子结果必须写进 rollout 并在 `evaluate` 复现）；含红线逐条合规做法 + 判据/失败分支 + 零成本前置探针 + 替代手段对比（机制侧 O7 最贴病根） | [`docs/dice_pass_option_analysis_2026-09-18.md`](docs/dice_pass_option_analysis_2026-09-18.md) |
