...
| **★ 阵型机制（第一批）+ 碰撞减半卡位预注册** | 出生几何原**只有均匀圆环**、gamedata **无任何阵型字段** ⇒ 新增 `formation.py` 形状表（表外**逐值不变**，12 张未登记卡复算逐值相同）。tick0 重叠对 改前→改后：`Goblins(4)` **2→0**、`RoyalHogs(4)` **4→0**；飞桶落塔从「被塔矩形推出」改成**塔后中央 + 上方左右角对角线**。★ 卡位实验**已预注册未跑**（`DarkWitch` **出生即在场上可被卡**；唯一差异 = e1 位移有无 `*0.5`） ｜ [`docs/formation_2026-09-19.md`](docs/formation_2026-09-19.md)、[`docs/collision_half_block_test_2026-09-19.md`](docs/collision_half_block_test_2026-09-19.md) |
