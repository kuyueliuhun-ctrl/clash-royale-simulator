...
| **★ 攒费意图动作（intent-save）：已实施（默认关）** | 用户拍板**扩参**。`STOP` **尾部追加** `SAVE(slot)×4`+`CANCEL`；★ **承诺期把花费整块掩掉**（只剩 保持/CANCEL）——不这么做则 34 帧合取 `p^k` 回归、机制价值归零（**与预注册「掩码零改动」的偏差见 §8**，【R13】重新适用）。保持**不需逐帧重抽**；落点**执行帧**才定；观测 +6。实测**参数 +2,053**。默认关**逐位旧行为**（selftest 断言：无 pending 时掩码逐位相同）。J1 = 20k 内 ≥1 条「≥34 帧意图后真打出」（基线 0）——**判读脚本未写，跑前必须补** ｜ [`docs/intent_save_prereg_2026-09-19.md`](docs/intent_save_prereg_2026-09-19.md) §8；全文 → 分册 **§18** |
