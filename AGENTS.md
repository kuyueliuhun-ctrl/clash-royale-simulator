...
| **★★ 用 FL 的 IL_Replay 做模仿学习（IL）**（**已跑完 + 已判读**） | 分片 0（5000 局）→ **47,715 条**人类标签、**卡牌经济对账 99.72%**（外部真值）→ **既有 BC 通路** 3 epoch → 留出 **8,877** 配对读数：**NLL 8.72→5.60**、**落点 0.86%→12.10%**、top-1 **32.87%→40.42%**。★ 结论 = **落点学到了、出牌选择没学到**（trivial「恒选槽 4」= **39.84%** ⇒ 预注册 40% 闸门**无分辨力**，实质 **F5 弱信号**）。J4 下游**未跑**（`train_follower.py:225` 解包 typo，首局结束必崩）。仪器 `_fl_il_extract` / `fl_il_to_bc` / `il_eval_holdout` ｜ [判读](docs/fl_il_2026-09-20.md)、[预注册](docs/fl_il_prereg_2026-09-20.md)；**C15 / O12** |
