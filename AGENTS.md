...
- **run 模式成本 + 两级评估 + 评估并行（2026-09-18 实测标定）**：纯训练 **27.4 步/s**（`n_envs=1`；
  ...
  同格点只评一次按大预算）。② **评估并行分片**（`--eval-workers N`，worker 屏蔽 CUDA、失败降级串行）⇒ 131 点 / 7100 局 **17.8 h → ~2.2 h**。
...
| `big_eval_every` / `n_eval_games_big` | **0 / 0**（2026-09-18 起；默认关 = 旧行为逐位不变） | 两级评估的大评估网格与局数（`rl/config.py`）；`--big-eval-every N --n-eval-games-big M` 开启。与小网格取**并集**：不整除时大点被**插入**（8000 vs 100000 ⇒ 131 点）。step 0 默认用大预算做低噪声基线 |
| run 模式评估并行 | `--eval-workers N`（**必须显式传**） | run 模式原本**完全忽略**该 flag（永远串行）；2026-09-18 起接线，但 `main()` 里在**未显式传**时强制 `eval_workers=1`（否则 dataclass 默认 16 会改变所有既有 run 命令的默认行为）。worker 屏蔽 CUDA、失败降级串行 |
...
| **2026-09-18 长跑工程（两级评估 / 评估并行 / 1M 步 run 模式）** ★：`_EvalScheduler`（小点保分辨率 + 稀疏大点降噪，两网格取并集）、run 模式评估分片并行（`--eval-workers`，默认仍串行、失败降级）、两个新回归测试 | 预注册 + 判据 [`docs/long1m_prereg_2026-09-18.md`](docs/long1m_prereg_2026-09-18.md) |
