...
| **O6** | **精确塔伤（触发式）已落地但默认关闭** | 旧近似闸门实测 BA **0.44~0.66**、4/6 组**劣于零信息基线**（`docs/threat_precise_probe_verdict_2026-09-14.md`）⇒ 改用引擎推演：`rl/threat_precise.py`（**上升沿触发 + 双向一次算完 + 保持寿命上限 20 s**），生产默认**关**（【R2】，关闭时逐位不变）。实测（镜像自对弈 3 seed）：触发率 2.1%~5.5%、摊销 **1.76~3.41 ms/帧**（预算 3.85）；**保持期内盘面变化一律不重算**（对方新放卡也不重算，347 帧事件 0 次推演）；保持时长中位 **10.5 s** 而推演视界只有 5 s ⇒ 加寿命上限 20 s（实测年龄-质量曲线在 ≥20 s 处交叉：BA 0.49 vs 粗糙 0.58，n=84 **未定**）。翻转版 `my_pressure` 3/3 seed 提升（+0.008/+0.136/+0.191），**原方向 `threat` 1/3 seed 转差（seed 11 −0.240，未归因）** | ①**端到端训练 A/B（开关 on/off，20k）——改任何生产默认值之前的前置条件**；②seed 11 归因（嫌疑：退出时刻度跳变，134 次退出中 50.7% 保持值 ≈0）；③真实对手池下重测触发率/摊销（当前余量仅 1.1~1.75×） |
