...
## G′ 三层可预测性对照 + G'-fix（2026-09-12，计划 v3 §3.13~§3.15，分支 `no-human-watch-B`）

### ① G′ 诊断（`scripts/diag_critic_ev.py --predict`）：先证功效，再下结论

新增三层对照（同批帧、同**按局分组**留出）：特征 = `raw`（12 维真值状态标量）/
`enc` / 二者拼接；标签 = 逐帧回报 / 局结果 / 局均回报 / 终局皇冠差；
另给**打乱局标签**的噪声地板。**功效对照**（目标就是特征的确定性函数）：
`raw → time`、`raw → hp_diff` 分组 R² = **+1.0000**，`enc → time` = **+0.9999**
⇒ 该测量能测出真信号，负值才可以解读。**做任何"预测不了"的结论前，先放功效对照。**

三条结论（`docs/diag_predict_fprime2.log`，50 局 / 16234 帧）：
1. **表征缺口（新，可修）**：`enc → 塔血差` 分组 R² = **−0.03**（逐帧口径 +0.41 是泄漏），
   而同一 enc → time = +0.9999。塔血此前只以 grid 每格通道存在，经
   CNN→grid_ln→enc_fc→relu→enc_ln→GRU 后线性不可解码；**策略头 slot/cell 只吃 h**
   ⇒ 决策通路没有可靠塔血输入。
2. **世界层面**：`raw → 局结果` 线性 −0.10 / MLP +0.06（地板 −0.17/−0.85）
   ⇒ **特权真值状态下局结果仍跨局不可预测**；只有终局皇冠差有弱信号（raw+enc +0.28）。
3. ⇒ **critic ≈ 常数是当前目标下的必然**，critic 侧应停（不要再换架构/加预算）。

### ② G'-fix 落地（观测+编码器，须 `--fresh`）

`tower_state` 9 维（我方三塔血量比 + 敌方三塔血量比 + 皇冠/3 + 圣水差/10）加进 obs，
`follower` 把它**追加在 `fused` 尾部**（`tower_dim=9`；`enc_dim`/`value_enc_fc` 同步）。
**必须 tail 追加**：插进 scalar 块会让 plan/belief 列后移 ⇒ `enc_fc` 的"前列拷贝"
兼容分支语义错位。`load_checkpoint` 对 `enc_fc.weight`/`value_enc_fc.weight` 走尾零兼容。
selftest `test_tower_state_observation`（语义/接线/改塔血 enc 必变/旧 obs 尾零）。

### ③ 20k 判读（`runs/gfix_20k`）：预注册判据 PASS，但**无收益、末点更差**

- **判据 PASS**：`enc → 塔血差` 分组 MLP R² **−0.26 → +0.69**（线性仍负是 LN+ReLU 的
  非线性；`enc → time` 两种架构都 ≈+1.00，说明探针口径没变）⇒ 塔血确实进 trunk 了。
- **critic 仍是常数且偏置更大**：离线 `v std=0.012`、`bias=−12.14`、
  `EV_global=−2.21`（F′ 是 −0.005）⇒ 独立确认"修表征救不了 critic"，
  并**预注册证伪**了"表征缺口是 critic 失败主因"。
- **末点崩塌**：@17500 三对照正常（0.562/0.725/0.475），@20000 全崩（0/40、0/40、1/39）、
  engagement 0.0；日志无爆炸指纹（gnorm≤7.03、entropy≥0.285）。
- **处置：分支保留、不并入 economy 预设/main**（机制层修复成立、效果层无证据且更差）。

### ④ 纪律（本轮第四次栽在"指标"上）

`train_solo.eval_and_write` 的 `stats["value_std_ratio"] = None` 写在 `if/else` **之外**，
会把刚算好的同窗口比值立刻清空 ⇒ 评估行 `vstd/rstd` 恒 None（`gfix_20k` 前两点实测丢了）。
**写"默认置空"时，确认它不在有效赋值之后无条件执行。**
