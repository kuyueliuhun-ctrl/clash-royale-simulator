...
- **噪声地板（40 局）**：胜率 **1σ≈0.078**、接敌率 **1σ≈±5pp**；**锚点相邻点 |Δ|≈0.225~0.240 ≈3σ**
  ⇒ 谷底只有 **1 个评估点宽（≲5000 步）**。
...
| `RAND_ANCHOR_EVAL_SEED` | **90000** | 锚点**固定**评估种子（每评估点重打同样 40 局 ⇒ 锚点是权重的确定性函数） |
...
| `_HIST_POOL_MAX` | 12 | hist 池上限；本目录 ckpt 优先 ⇒ **长 run 会把 `--hist-seed-dir` 的外部补种挤出**（40k 起池子全 12 个都是本 run 快照） |
...
| **O2** | **critic 拟合** | 结构问题已被排除（X2/X3），训练过程层面未解；注意：跨局回报不可预测 + MSE 最优解是条件均值，本身可能"无解" | 若重启：先在**监督设定**下确认可达上限（已有 +0.22~+0.29 的证据链），再谈 PPO 侧；**不要再靠加量**（X4） |
