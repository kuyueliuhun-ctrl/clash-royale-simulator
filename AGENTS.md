...
| **★ 多会话协作台账**（**多个会话同时改这个仓库：谁在改哪些文件**） | [`docs/agents/session_ledger.md`](docs/agents/session_ledger.md)：**A/B/C 会话表 + 文件占用表 + 冲突处理**；纪律 = **动手前先登记、只改自己那行、冲突即停**；★ 工作区硬约束：**勿 `git add -A` / `git checkout .`**（270 项未提交删除 = 09-20 目录精简，`docs/_archive/` 尚未进 git） |
...
| **★ 低费法术空砸豁免 + 手牌打分/信息分预注册**（C22） | 口径更正：**Poison = 4 费，不便宜**（上轮算进「便宜」是错的）。本仓 **27 张 `type=="spell"`**；「低费」= **cost ≤ 2** = {Zap, Snowball, Log, BarbLog}。新增 `SPELL_WHIFF_FREE_MAX_COST=2.0` + `_spell_whiff_gate_applies`，**只豁免 8h 空砸、9h 砸塔 EV 不动** ⇒ 空地 Zap 合法而「只罩王塔」仍非法（F1 不回退）。全法术位图 **432 张** A/B ⇒ **32 张变化 / 收紧 0 / 越界 0**；标准 128 张**逐位全等**；回归 **32/32**。W2/W3 预注册已落（W2 需先修 `v[PLAN_DIM-4]` 坑 + `--fresh`）。｜ [预注册](docs/il_whiff_handscore_prereg_2026-09-22.md)；**C22** |
| **★ 解耦组合臂：恒等式被证伪 ⇒ 不是有效的解耦检验**（C23） | τ 判决把 act-AUC 从 **0.7354** 压到 **≤0.6713 < J8.1 的 0.6948**（根因 = **R00 全 39,761 帧 `argmax==STOP` 0 次**）；连续分数口径 = **同义反复**。实现：runner 增 `--gate-ckpt/--gate-threshold`，`src/` 零改动，**F0 恒等回归 PASS**。结果：**plays/game 34.70**（⇒ **K1 FAIL**）、圣水中位 4.36（K3 PASS）。★ **`playable_rate` 0.1037→0.8900** ⇒ 状态分布巨变、三轴全朝人类动但**都没到** ⇒ **off-policy 不匹配**、**原理上无法隔离「头耦合」**。**下一步：Stage 2 联合训练独立 act 头**。｜ [§6](docs/fl_il_il2_prereg_2026-09-22.md)；**C23** ｜ 全文 → 分册 **§27** |
| **★ 独立 act 头：离线双向改善、闭环未破坏；P4① FAIL = 配对池选错**（C24） | 拆头后 stop/play 两侧 NLL **不可与共享头直接比**（构造性）⇒ 必须用**嵌入 6 类口径** `p₆'=p₂(ACT)·p₅`（可执行断言 `max|Δ|=6e-07`）。实现：`act_head(2)` + `slot_head(5)`，`SUB_SPACE_DIM` 使 `sub_emb`/`enc_fc` **不变**；旧路径分离实现 ⇒ **F0 `29/29 逐位相同`**；回归 **15/15**。同池同 seed：**Δact-AUC +0.0224 / ΔNLL −0.0170**；**出牌/局 33.90 ⇒ P4① FAIL**，但 `stop_when_playable` **两臂均 0.0000**、`playable_rate` **0.1022 vs 0.1037** ⇒ **池无 save 帧、检验无效**（**实施错误**）。｜ [§7](docs/fl_il_il2_prereg_2026-09-22.md)；**C24** ｜ 全文 → 分册 **§28** |
| **★ 拆头路线关闭：头耦合不是绑定约束**（C25） | 修正配对（与 **R10 同池** 77,486、同 seed，唯一变量=架构）下拆头**没**解开耦合：**ΔNLL +0.0697**（**P6 FAIL**）、**出牌/局 30.70→33.20**（**P4① FAIL**，更远）。★ **方向才是决定性的**：`stop_when_playable` **0.2228→0.0879** —— 专用 act 头让模型**更少**在买得起时不出，与假设**相反**。P1/P3 均 PASS ⇒ 按 §7.5 判 **共享头诊断不成立**。⇒ **帕累托前沿是池构成的性质，不是头架构** ⇒ 下一刀转 **数据/标签分布** 或 **机制侧 O7/O11**。开关保留（默认关）。｜ [§7.13](docs/fl_il_il2_prereg_2026-09-22.md)；**C25** ｜ 全文 → 分册 **§29** |
| **★ 手牌打分+前期信息分：特征被用了、端到端判据未过**（C26） | plan 尾追加 **17 维**特征。★ **对照组必须同形状**（`manual_seed` 先于建网 ⇒ 58 vs 75 维初值全不同）⇒ `plan_extras_zero=K` 抹尾列 ⇒ 唯一变量 = 那 17 列有无信息。**J-W2.3 FAIL**（ΔNLL 均 **+0.036**、Δact-AUC 均 **+0.0051**）⇒ 未注册第三分支 ⇒ 不采用/不回滚/默认关；ΔNLL 极差 0.206 ≫ 均值 ⇒ **不许写「有害」**。★ **归因强阳性**：`ΔNLL(perm)=+0.0811≈28×抖动`、负对照 **0** ⇒ 网络真在用 ⇒ **瓶颈不在特征**。**W3 判据饱和**（`known_opp` 中位全 4.00）⇒ PASS 空。｜ [§6.9](docs/il_whiff_handscore_prereg_2026-09-22.md)；**C26** ｜ 全文 → 分册 **§30** |
| **★ 觉醒 S1：`--evo-slots` 让觉醒在重建战局里真触发，并**立刻暴露引擎觉醒路径两个真崩溃**（C27） | 牌组 `-ev*`→`set_evolution_slots`（声明须**周期表 ∧ `evo_raw` 双齐备** ⇒ `-hero` 不接线）。★**③ 生效性 PASS**：**445 次触发**、191/200 局声明（=100% 可转换局）。★**① 能声明率 0.9204 FAIL**：差集**恰 1 张卡** `elite-barbarians-ev1`（本仓无觉醒快照）。★**② labels −6.42%/errors 9 FAIL，归因明确**：剔除 9 局崩溃后 **−1.65%** ⇒ 引擎觉醒路径**从未执行过**，`statsTags` 被当机制值 ⇒ `Witch_EV1`/`GoblinDrill_EV1` 必崩 ⇒ **停手：不做 S2/S3**、不回滚。｜ [§6–§10](docs/il_evo_prereg_2026-09-22.md)；**C27** ｜ 全文 → 分册 **§31** |
| **★ O-E2 已修：跳过 `statsTags` ⇒ S1 的 ②③ 兑现**（C28） | 一行 `if k=='statsTags': continue`（不动触发/数值/掩码）。★ **A/B 复算**（34 张 evo）：**其余 32 张逐键逐值相等**，变化**恰 2 处** —— `Witch_EV1.spawnPauseTime`→**7000**、`GoblinCage_EV1_TEMPNAME.deathSpawnCount`→**1**。★ **200 局重跑**：`errors` **9→0**、② labels **−6.42%→−1.30% PASS**、③ 触发 **445→482**、声明 **191→200/200**（首现 `Witch`/`GoblinCage`）。★ 回归 **28/28**。★ ①仍 **FAIL 0.9239**：唯一原因是 `elite-barbarians-ev1` 本仓**无觉醒快照**（数据缺口）⇒ 须另行预注册（O-E1）；**S2/S3 未启动**、默认关。｜ [§11](docs/il_evo_prereg_2026-09-22.md)；**C28** ｜ 分册 **§32** |
