...
> ★ `audit_intent_save.py`（攒费意图 intent-save 的 **J1 判读仪器**，`docs/intent_save_prereg_2026-09-19.md` §4/§8.5）用法：`./.venv/Scripts/python.exe scripts/audit_intent_save.py --ckpt <ckpt.pt> [--games 8] [--max-frames 400] [--seed 0] [--deterministic] [--deck solo|vanilla] [--allow-arch-mismatch] [--allow-inapplicable] [--json [路径]]`（`--ckpt` 缺省时须 `--random-init`；非 11-option ckpt 默认报错退出）。

> ★ `probe_intent_target.py`（**攒费意图目标归因**仪器：攒的是哪一张牌 / 等了多少帧 / 还差多少费；`docs/intent_target_attribution_2026-09-20.md`）用法：`./.venv/Scripts/python.exe scripts/probe_intent_target.py --ckpt <11-option ckpt.pt> [--deck solo|vanilla] [--games 20] [--max-frames 400] [--seed 0] [--json [路径]]`。★ **`--deck solo`（缺省）才是训练卡组**；`vanilla` 只为复现旧读数（原版 8 卡无 6 费牌 ⇒ J1 空判据）。
