...
**合计 112 个 `*.py`**（另有 `scripts/rl/*.py` **11 个**：`start_rl.bat` 依赖其位置的入口包装脚本，**不参与**本分类）。

> ★ `audit_intent_save.py`（攒费意图 intent-save 的 **J1 判读仪器**，`docs/intent_save_prereg_2026-09-19.md` §4/§8.5）用法：`./.venv/Scripts/python.exe scripts/audit_intent_save.py --ckpt <ckpt.pt> [--games 8] [--max-frames 400] [--seed 0] [--deterministic] [--allow-arch-mismatch] [--json [路径]]`（`--ckpt` 缺省时须 `--random-init`；非 11-option ckpt 默认报错退出）。
