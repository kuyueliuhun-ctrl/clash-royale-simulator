...
def main() -> int:
    ...
    W("6. **独立唱反调校验**：另派一个无上下文的子代理只读源码、专找编造与错误；"
      "它找出 9 条不符项（含 1 条「编造 grep 结论」级），已逐条更正并全部记录在 §4.5。")
    W("")
    W("### 0.3 阅读约定")
    W("")
    W("| 标记 | 含义 |")
    ...
    if rc:
        ...
        W("")
    # 4.5 独立校验与更正记录
    W("### 4.5 独立校验与更正记录（唱反调抽样反查）")
    W("")
    W("2026-09-14 由**独立子代理**做「唱反调」校验：它没有本次对话的上下文、只读源码与三份文档、禁止上网，"
      "并被明确要求找出编造/夸大/错误。抽样是确定性的（每 24 条符号取 1、抽满 40 条；两份专题文档每 40 行取 1 行），"
      "判定全部基于亲读源码行号，可执行断言实跑取值。报告见 `docs/_survey/verification_report_2026-09-14.md`。")
    W("")
    W("**它发现 9 条不符项；本文档与两份专题文档已逐条更正**（更正方式一律是：改素材 → 重新机械合并，不是手改成品）：")
    W("")
    W("| # | 严重度 | 问题 | 更正 |")
    W("|---|---|---|---|")
    W("| 1 | **★编造级** | `core.BlankEntity` 条目写「仓库内 grep 只命中本文件 ⇒ 全仓无使用点」——**与事实相反**"
      "（`battle.py:1` 导入、`:1670` 与 `:2964` 两次实例化）| 已改为正确事实，并标注原文已作废 |")
    W("| 2 | 高（易被下游照抄） | 训练文档称 `env_wrapper._DEFAULT_REWARD` 与 `config.DEFAULT_REWARD` 是「同值副本」"
      "——实跑为 **16 键 vs 17 键**，差 `draw_penalty` | 已改为「**不是**同值副本，改奖励常量必须同时改两处」 |")
    W("| 3 | 高（语义误导） | `Entity.take_damage` 被写成「先扣护盾再扣血」——实为"
      "**有盾只扣盾、无盾才扣血，伤害不溢出** | 已改为源码语义并附两行源码 |")
    W("| 4 | 中 | `aggressive` 预设 reward 键数在同一行自相矛盾（先写 7 键后写 6 键）| 已改为精确表述："
      "**字面字典 6 键、解析后 17 键（与 `DEFAULT_REWARD` 合并）** |")
    W("| 5 | 低 | 把 `env_wrapper.py:684` 称作 `compute_reward` 调用点——实为 **678** | 已改 |")
    W("| 6 | 低 | `test_m2.py` 的 `check()` 调用点「约 60 处」——实为 **74** | 已改，并写明复算方法 |")
    W("| 7 | 低 | `derive_evolved_stats` 特殊字段「18 个」——元组实为 **17** | 已改 |")
    W("| 8 | 过度保守 | `spawn_building` 第 5 个位置实参被标「无法确认」——读 `battle.py:1234` 即知是 `persistent` | 已补全为可确认 |")
    W("| 9 | 过度保守 + 文档内自相矛盾 | 引擎文档 A.7 第 4 项把同一文档 §B.8 已回答的「RL 掩码层是否补齐」标为无法确认 | "
      "已改为「见 §B.8」，并挂上逐格对账日志 |")
    W("")
    W("**校验方给出的正面读数（客观，非客套）**：跨文件行号引用极准——专项攻击 15 条行内引用 **15/15** 正确；"
      "训练文档 **15/15**、引擎文档 **11/11** 全对（含最易错的数值常量 / 坐标区间 / 阶段顺序 / 函数名四类）；"
      "`_spell_deals_damage` 两面对照表**实跑亦对**。")
    W("")
    W("**校验方自陈的局限（照录，禁止外推）**：937 条符号只抽 **4.3%**；`#####` 子符号（568 条）与专题文档 §B.6–B.9 未覆盖；"
      "只做静态读取 + 少量纯计算实跑，**未**启动训练或对局 ⇒ 「未发现更多」**不能**外推为「全文正确」。")
    W("")
    W("> 除人工唱反调外，本文档另配**可复算**的反查工具 `scripts/_survey_reverse_check.py`（见 §4.4）："
      "脚本负责「每个引用是否回源」，人工负责「语义有没有被说反」，两者互补。")
    W("")
    # 5) 全局符号索引
    W("## 5. 全局符号索引")
    W("")
