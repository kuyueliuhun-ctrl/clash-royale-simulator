...
| `et_solo100k_2026-09-18.md` | **`et_solo100k` — 用户要求的 100k 局面圣水交换干预长跑（两臂）记录（2026-09-18，进行中）**：`A_et = economy_et`（`engagement_trade=0.5` 与 `edw` 同汇率）+ `B_ctrl = economy_etm`（仅 `measure_only` 差），均带 `--adv-inert-probe`；**两次发射**（第 1 次漏探针 ⇒ step 7547 作废留证）；`eval@0` 三层读数（结果 `0.650±0.107` / 行为层「全帧圣水≥6 = **0.00%**、Xbow 出手 **0**、部署前圣水中位 **3.250**」/ 门禁 `Δρ(塔血) +0.024`、`ρ(τ,φ) +0.443`）；仪器全部在本 run 自己的数据上跑通；【R1】预检与「内联 `$_.CommandLine` 假阴性」教训 | 预注册 §11.13 / **§11.13.6 / §11.13.7**（两处测量侧更正）；数据 `runs/et_solo100k/`、`runs/_discarded_et_solo100k_noprobe/` |
