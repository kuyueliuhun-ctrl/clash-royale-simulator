...
>
> ⇒ **方案 §6 的 21 项现在全部有结论**（逐项核对过，不含糊）：
>
> | 类别 | 数 | 项 |
> |---|---:|---|
> | **完全完成** | **15** | T0-1..4、T1-1..5、T2-1、T2-2、T2-5、T2-7、T2-8、T3-4 |
> | **部分完成（另一半判定不做）** | **3** | **T2-3**（判定语义半簇已搬 / 评估并行半簇不做）、**T2-6**（分目录不做 / 交了替代物 `scripts/README.md` + ⑩）、**T3-2**（Stage A 取消 cwd 契约 / Stage B 搬文件不做） |
> | **判定不做** | **3** | **T2-4**（无逐位对账）、**T3-1**（方案 §7.2 #3 自拒）、**T3-3**（方案 §7.2 #2 自拒；T2-8 已解决其动机） |
>
> 每一条「不做」都带**证据**（不是偏好）：见下 §1–§7。
