...
> ## ★ 最终状态（用户已就其中两项拍板）
> 用户看过本文件后**只勾选了两项**要强行做 ⇒ **这两项已完成并推送**：
>
> | 项 | 分支 | 关键证据 |
> |---|---|---|
> | **T2-7** 抽奖励簇 → `rl/reward.py` | `structure-t2-7-reward` / `efc97f8` | `env_wrapper.py` **812 → 598 行**；**纯搬运逐字节相同**；**数值逐位 A/B：`compute_reward` 4,000 例 0 不一致**、helper 5,021 例全同；**全量套件 100 通过 / 0 失败** |
> | **T3-4** 统一 `sys.path` 引导 | `structure-t3-4-bootstrap-order` / `ff09c70` | 先量出**参数层面无可统一**（135 条语句 ~10 种参数表达式）⇒ 改为交付 **⑪ 门禁**（引导先于产品 import），判别力已断言 |
>
> **其余 4 项（T2-3 评估并行半簇 / T2-4 / T3-1 / T3-3）按下面的判定保持「不做」**；
> 另 **T2-6** 亦判定不做并已交替代物（`scripts/README.md` + 检查 ⑩）。
>
> ⇒ **方案 §6 的 21 项现在全部有结论**：完成 17、判定不做 4（各有证据，见下）。
...
## 2. T2-4 `train_solo` 抽函数 —— **【R2】碰不得 + 无对账手段**（用户未勾选 ⇒ 不做）
...
## 3. T2-7 抽奖励簇 → `rl/reward.py` —— ✅ **用户拍板「做」并已完成**（`structure-t2-7-reward`）
...
## 4. T3-1 拆 `battle.py` —— **方案 §7.2 #3 明确「拒绝做」**（用户未勾选 ⇒ 不做）
...
## 5. T3-3 迁 `tests/` + pytest —— **方案 §7.2 #2 明确「拒绝做，默认不做」**（用户未勾选 ⇒ 不做）
...
## 6. T3-4 统一 `sys.path` 引导 —— ✅ **用户拍板「做」并已完成**（`structure-t3-4-bootstrap-order`）
