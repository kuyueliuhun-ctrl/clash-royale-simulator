...
### 6.9.7 用户指定口径：卡牌使用率对比（`il_card_usage.py`，9 跑各 12,000 帧）

**偏好口径** `cond_X(c) = P(X 打 c | c 在该帧手牌中)`；聚合读数
**`U = pref_capture_policy`**（人类偏好被复现的比例，1 = 完全复现）：

| 臂 | `U`（3 seed 均） | `policy_stop_rate`（3 seed 均） |
|---|---|---|
| HS17（对照） | **0.7771** | 0.7826 |
| HS04（W2） | **0.7601** | 0.7796 |
| HS00（W2+W3） | **0.7780** | 0.7913 |

人类侧：`stop` = **0.7779**（整套留出 39,652 帧的实测值，见 `holdout_*.json` 的 `human_stop_rate`）。
⇒ **三臂的 `U` 与 `stop` 率几乎重合**（`U` 极差 0.018、`stop` 极差 0.012）⇒ **与 J-W2.3 的结论一致：
W2/W3 没有可分辨地改变「出牌偏好画像」**。

**逐卡读数（节选，`cond_policy` 的 3-seed 均值）** —— 真正大的差距是**人类 vs 模型**，**不是臂间**：

| 卡（在手帧数） | 人类 | HS17 | HS04 | HS00 | 读法 |
|---|---|---|---|---|---|
| Cannon (3130) | 0.0380 | 0.0280 | 0.0277 | 0.0224 | 模型**少出**（臂间 −0.006） |
| Princess (1529) | 0.0876 | 0.0447 | 0.0429 | 0.0355 | 模型**少出约一半** |
| GoblinBarrel (1733) | 0.0514 | 0.0242 | 0.0185 | 0.0160 | 模型**少出** |
| MightyMiner (1673) | 0.0622 | 0.0373 | 0.0351 | 0.0319 | 模型少出 |
| **Skeletons (1474)** | 0.1262 | **0.1723** | 0.1698 | 0.1603 | 模型**多出** |
| **BarbLog (1697)** | 0.0554 | **0.0699** | 0.0797 | 0.0684 | 模型**多出**（且本仓 BarbLog 部署**恒被引擎拒**，见 §5 O-c） |
| Rocket (1383) | 0.0123 | 0.0241 | 0.0243 | 0.0260 | 模型多出 |

⚠️ **两条【R10】限制**：① `--limit 12000` 取的是**文件序前 12,000 帧**（含 play 与 save），
不是随机抽样 ⇒ 与整套留出的读数（0.7779）**不是同一批帧**，只作**同口径横向比**；
② 本表**不支持**任何「W2 让模型更会避开关键牌」的说法 —— 臂间差（0.001–0.008）**与 seed 噪声同量级**，
唯一稳定的模式是**人类 vs 模型的系统性偏好缺口**（模型少出 Princess/GoblinBarrel/Cannon、多出 Skeletons/BarbLog），
**该缺口在本批前后都存在、未被 W2/W3 改变**。
