...
async function extractPage(url, idx) {
  ...
  const out = `/mnt/e/clash-royale-simulator-main/docs/_page_b05_${idx}.txt`;
...
(async () => {
  ...
  for (let i = 0; i < urls.length; i++) {
    try { await extractPage(urls[i], Number(process.env.B05_IDX0) + i); } catch (e) { console.log(`[${i + 1}] ${urls[i]} 失败: ${e.message}`); }
  }
})();
