...
| **FL 回放的卡组能不能在两台引擎里都建起来** | **能，覆盖很高**：抽 **120 局真实 FL 回放**（`/tmp/il_actions.parquet`，`_il_replay_extract.py --n 120`）逐卡判定：<br>• 我们引擎：**120/120 局**可解析（唯一解析失败是 `void`，4 个卡位 —— 与既有结论"卡覆盖 177/178，只缺 `void`"一致）<br>• 上游：**114/120 局（95%）**的 16 张卡全部落在它的 150 卡表里；真正缺的只有 **`vines`(4 槽) 与 `ronin`(2 槽)** 两张新卡 | 本次实测（见下方命令块） |
| ⚠️ **变体键的真实语义（必须先说清）** | `resolve_card('bats-ev1')` → **`None`**、`resolve_card('magic-archer-hero')` → **`None`**：**`-ev1`/`-hero` 不是卡名而是"变体标记"**，仓库既有映射（`replay_feasibility_probe.py:67-83`）的做法是**去掉后缀解析基卡**、变体另作标志位。<br>⇒ 上面的 114/120 是**"变体降级为基卡"**口径下的数：**我们引擎能出觉醒/英雄形态，上游只能出基卡**（它没有这些表项）。**这不是"上游跑不了这局"，而是"同一局两边出的牌形态不同"** —— 这条必须在判据里表态（见 §6 口径 A/B） | `resolve_card` 实测：`the-log→Log`、`ice-spirit→IceSpirits`、`magic-archer→EliteArcher` 都在上游表中 |
| ⚠️ **我自己的一次错误（留档）** | 我第一版用"kebab-case 直接转 CamelCase"的**自制映射**，得到 **0/120**、"65 张卡上游都没有"——**错的**：`the-log` 的真名是 `Log`、`ice-spirit` 是 `IceSpirits`。改用仓库既有的 `resolve_card` 后真实数字是 **114/120**。⇒ **不得引用那个 0/120** | 错误版的"缺失卡"里含 `the-log`/`ice-spirit`/`fire-spirit` 等**上游明明有的**卡 |

**本节用到的命令（只读，可复跑）**：

```bash
# 1) 提取 120 局（WSL python3 有 pyarrow；Windows venv 没有）
cd /mnt/e/clash-royale-simulator-main
/usr/bin/python3 scripts/_il_replay_extract.py --actions /tmp/il_actions.parquet \
    --replays /tmp/il_replays.parquet --n 120 --out /tmp/il_120.json
# 2) 卡表（上游需 fastcore 才能 import card_utils；此处只读 gamedata 亦可，但直接 import 最准）
```
...
3. **卡表差（已实测，量级很小但性质重要）**：真正两边都缺的只有 `vines`/`ronin`（6/120 局受影响）；
   但**觉醒/英雄形态只有我们有**（上游表里没有 `*_EV1`/Hero 表项）⇒ 同一局**两边出的牌形态不同**。
   ⇒ 这必须**预先选一种口径**并在写作时声明（【R17】）：
   **(I) 两侧都降级为基卡**（差异只剩引擎物理/寻路，代价 = 丢掉觉醒/英雄机制）；
   **(II) 各按各的解释跑**（更接近"谁的整套模拟更像真机"，代价 = 把卡牌机制差混进分数）。
...
**卡牌口径（必须二选一，先定）**：`CARD_MODE = base-downgrade | per-engine`（§5.2-3）。
**样本**：FL 回放中"**两侧引擎都支持其牌组**"的局（base 口径下实测 **114/120 = 95%**；剔除 `vines`/`ronin` 局，占比要报）。
