...
## §9 更新（2026-09-18）：评估曾是本模式最大的墙钟项 ⇒ 已解决

本文件 §6 的未决项里，"run 模式评估是**串行**的"（原 `eval_round_robin` → 逐 pair 逐局，
且**完全忽略** `--eval-workers`）是实际部署该模式时最大的工程障碍：实测**单局评估 ≈8~10 s**
（纯 Python 引擎，1 决策帧 = 30 tick），而 **训练**只要 27.4 步/s ⇒ 一次 1M 步长跑的
7 100 局评估（= 17.8 h）比训练本身（10.1 h）还贵。

2026-09-18 落地两件事（判据与成本标定见 [`docs/long1m_prereg_2026-09-18.md`](long1m_prereg_2026-09-18.md)）：

1. **两级评估** `--big-eval-every` / `--n-eval-games-big`：小点保曲线分辨率、稀疏大点压噪声；
   两网格**取并集**（不整除 ⇒ 大点被插入），同格点只评一次按大预算。**默认 0 = 关 = 旧行为逐位不变**。
2. **评估分片并行** `eval_round_robin(..., n_workers=N)`：把全部 (pair, game) 分给 N 个 spawn worker
   （纯 CPU、屏蔽 CUDA），**Elo/PFSP 仍由父进程按 (pair, g) 规范顺序逐局补记** ⇒ league 状态演化与串行一致
   （`test_eval_round_robin_parallel_equivalence` 断言 `n_games=1` 时并行与串行的 Elo + 录像**逐位相同**）。
   失败自动降级串行重算（不记部分结果）。**默认仍是串行**——只有显式传 `--eval-workers` 才启用。

⇒ 7 100 局从 ≈17.8 h 降到 **≈2.2 h**（12 worker）。**§6 的"评估串行"未决项据此关闭**；
仍然未变的是 §4 的核心 caveat：对手全是 mask-随机脚本（无自对弈、无防守脚本），
所以本模式的 Elo 曲线是"对**固定仪器**的绝对强度读数"，不是自我对弈意义的强度。
