...
### 3.15 G′-fix 20k 验证（`gfix_20k`，2026-09-12）：**预注册判据 PASS，但训练收益为零、末点更差**

协议与 F′ 完全一致（同 seed / 同 hist 种子目录 / `--ppo-epochs 4 --ppo-minibatch 32
--ppo-shuffle`），**唯一变量 = `tower_state` 9 维标量**。70 局、循环 2307.8s、
0 降级 / 0 Traceback。

| step | ES 池化 EV（**更新前**口径） | h_std | n_abs | 镜像胜率 | vs baseline0 | vs baseline_prev | vs baseline_rand |
|---|---|---|---|---|---|---|---|
| 2500 | **+0.2824** | 0.0404 | 0.550 | 0.325 | — | — | — |
| 5000 | **+0.1574** | 0.0493 | 0.622 | 0.075 | — | — | — |
| 7500 | −0.1039 | 0.0539 | 0.640 | 0.425 | — | — | — |
| 10000 | +0.0387 | 0.0592 | 0.650 | 0.400 | — | — | — |
| 12500 | +0.0045 | 0.0680 | 0.636 | 0.575 | — | — | — |
| 15000 | −0.0120 | 0.0650 | 0.667 | 0.350 | 0.650 | 0.300 | 0.300 |
| 17500 | **+0.6597** | 0.0525 | 0.665 | 0.575 | 0.562 | 0.725 | 0.475 |
| 20000 | −0.1062 | 0.0667 | 0.772 | **0.075** | **0.000** | **0.000** | **0.025** |

**① 预注册判据 PASS**（`docs/diag_predict_gfix.log`，50 局 / 16664 帧，按局分组留出）：

| 探针 | F′（旧架构） | G′-fix（本 run） |
|---|---|---|
| `enc → CTRL:hp_diff` MLP | −0.2632 | **+0.6865** |
| `enc → CTRL:hp_diff` 线性 | −0.0288 | −1.3199 |
| `enc → CTRL:time` MLP | +0.9993 | +0.9995 |

⇒ **塔血状态现在真的进 trunk 且可解码**（线性仍负是 `enc_ln` LayerNorm + ReLU 的非线性
所致；MLP 才是"信息在不在"的判据，而 `time` 这条对照在两种架构下都 ≈+1.00，
说明探针口径没变）。**观测/表征缺口假设成立并已修复。**

**② 但 critic 依旧是常数，且这次偏置极大**（离线 50 局）：
`v: mean=3.161 std=0.012`（range 3.14~3.19）vs `R: mean=15.305 std=8.156` ⇒
`bias=−12.143`、`EV_global = −bias²/Var(R) = −2.2146`（**历史最差**，F′ 是 −0.005）。
即：**修好表征并没有让 critic 拟合** —— 这是 §3.13 ③"逐帧回报跨局不可预测"的
**独立确认**，同时构成"表征缺口是 critic 失败主因"这一假说的**预注册证伪**（已排除）。
（注：训练内建池化 EV 在窗口内是 −0.11~+0.66，与离线 −2.21 的差异来自窗口/样本，
两者都说明"不比常数好"。）

**③ 末点像一次策略崩塌，且没有任何收益证据**：@17500 三对照正常
（0.562 / 0.725 / 0.475），@20000 全部崩（**0/40、0/40、1/39**）、
`engagement 0.0` / `ghost 25.5`；日志里没有爆炸指纹（`gnorm` max 7.03、`entropy` min 0.285），
所以这更像 cycling 的极端值（F′ 也出现过单点 0.075，但它的末点是 0.900）。
**单点不能定论崩塌是否可复现**，但**没有任何一个指标支持 G′-fix 有益**。

**④ 结论**：G′-fix 保持为**实验分支 `no-human-watch-B`，不并入 economy 预设 / main**。
- 机制层：塔血可解码（值得保留在分支上，作为后续"观测补全"的起点）；
- 效果层：EV 更差、末点崩塌风险 ⇒ 不能当默认架构；
- 顺带确认：**EV 判读从今以后用"更新前"口径**（分支 A 的修复），F′ 与 G′-fix 若要
  干净对比，必须把 F′ 用同口径复跑一遍（F′ 当初的 +0.10~+0.56 是 in-sample，不可直读）。
