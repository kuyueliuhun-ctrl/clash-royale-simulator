...
| `scripts/run_selftests.py`（2026-09-14，**2026-09-19 加 `--order-check`**） | 按名跑**子集** selftest（【R19】默认用法；不改 `selftest.py`）。**`--order-check`**（Tier 1 · T1-3）额外报**三种顺序的差集**：**定义序 / `main()` 手工序 / `dir()` 字母序** —— 三者**顺序两两不同**（集合相同），而 `--list` 是**字母序** ⇒ 用 `--list` 挑子集时"跑的先后 ≠ 全量"。判据只要求「① 定义集合 == `main()` 调用集合」（有测试没被登记 = 它从不跑），②③ 明示**不要求**成立 |
| `src/clasher_new/rl/io_bootstrap.py`（2026-09-19，Tier 1 · T1-1） | **UTF-8 兜底单一实现**：`force_utf8_stdout()` = `stdout`+`stderr` 两路 + `errors="replace"`、幂等、吞异常、**零依赖**（可在脚本最早期调用）。收敛前有 **3 份**（`run_league`/`dashboard` 强版逐字相同但 docstring 异、`probe_value_ln` **弱版**只碰 stdout 且无 replace）⇒ 现为 **1 处定义 + 3 处别名**（调用点一行未动）。⚠️ **未收敛的调用点仍在**：全仓 **61** 个内联 `stdout.reconfigure` 块，只收敛了有 path guard 的 **29** 个（其余 17+15 需新增 `sys.path` 项或形态不同 ⇒ 不做，理由见 `docs/structure_tier1_2026-09-19.md` §1.4）——**这 32 个文件的 stderr 乱码洞仍在**（实测证据：`duel_search.py` 的 traceback 在 GBK 下是乱码） |
| `scripts/selftest_io_bootstrap.py`（2026-09-19，T1-1 回归，【R8】） | `io_bootstrap` 的回归测试（**12/12 PASS**）：行为 9 项（两路 / 参数逐字 / 返回计数 / 异常流不抛 / 幂等 / 真实流切 utf-8 / **反向验证：中文+emoji 打得出来**）+ **单一来源 3 项**（**复用** `_structure_check.check_utf8`，不造第二套口径，【R17】）。独立跑：`python scripts/selftest_io_bootstrap.py` |
| `scripts/_structure_check.py`（2026-09-19，Tier 0 · T0-2） | **只读结构核对器**（7 项：分区规模 / UTF-8 兜底收敛度 / **引擎→rl 反向边** / 死件复活 / 硬编码绝对路径 / `rl` 函数体内 import 两套口径 / selftest 登记数）。**纯 `ast` 文本解析、不 import 产品代码** ⇒ 不受 `card_utils` 的 cwd 契约影响；单项失败记 `error` 不抛。`--selftest`（合成目录、零仓库依赖）**13/13 PASS**，含**反向验证**（造一个非法反向边必须被报出、修掉后必须归零）。退出码：③④⑦ 任一失败 ⇒ 1 |
| `scripts/_converge_utf8_bootstrap.py`（2026-09-19，T1-1 的一次性 codemod） | 把内联 UTF-8 兜底块换成 `from rl.io_bootstrap import force_utf8_stdout` + 调用。**只动"该块之前已有 `sys.path.insert`"的安全子集**（29/61），其余**跳过并打印清单**。`--dry-run` 可复现当时的判定 |
