...
| **★ S2 仪器实测与门禁判读**（2026-09-18）：`docs/s2_instrument_2026-09-18.md` —— 不变量 **6/6 PASS**；**预注册首选 P3 被推翻**（`τ ≤ kill_credit` 恒成立 ⇒ 只是已有 Φ 击杀项的重新加权，非新信息）；**P1/P2 否决**（P1 量纲是 `ΔΦ` 的 ~80 倍、ρ(胜) 5/5 批为负）；**唯一候选 = 新构造 P4b**（方向 5/5 批稳、幅度未超批间散布）；缺口实测 **20.6%** 的窗口「钉住但没打死」其 `Trade` 中位 0 / 均值 **−0.43**；★ **5 个口径缺陷**（身份判据 span=247 / `K` 不可辨识 / 零和须两侧对称 / `crown` 返回"让出的皇冠" / `tower_term` 与 `ΔΦ` 的重复计数）；**门禁判「有候选、无判决」** ⇒ S3 不得按原样开跑 | 仪器 `scripts/offline_engagement_trade.py`、测试 `scripts/selftest_offline_engagement_trade.py` |
