...
| **`intent_ab_100k`**（`solo` 模式，**两臂** 各 100k；2026-09-20 **已跑完**） | **用户发令的 intent-save 机制干预长跑**：`A = intent_off_100k`（状态 quo，6 option）＋ `B = intent_on_100k`（`--intent-save`，11 option）——**唯一变量 = `--intent-save`**，两臂 `--fresh`、`seed 0`、**顺序跑**、`--eval-workers 8`（【R1】开跑前实测可用提交 10.51 GB < 12 GB 门 ⇒ 按 `check_commit.py` 自身建议降档，两臂同档），各 **14/14** 评估点与回放、**0 降级 / 0 Traceback**、阳性完成门禁两臂均 OK（`[done] solo`＋`eval@100000`）、`ALLDONE 04:00:02`。 单变量在**入参层与产物层两处**核实（A `slot_head=(6,128)`/`intent_options=False`；B `(11,128)`/`sub_emb=(128,13)`/`enc_fc=(128,2737)`）。 **判读（`docs/intent_ab_100k_judgment.md`）：主判据 J1 = FAIL（0 条）**，但**不是「轨迹从未被采样」**——机制被大量使用（`held`≈**37% 帧**、`set=2674`），失败在**长保持从未完成**（`fired_held ≤ 6 帧`）＋ **`ready=17→fired=2`**（攒够却不出）；**三条预注册失败分支全未触发**（F2 前件成立但兑现有效：MiniPekka 4 费 → 206 塔伤 / 3 杀）⇒ 实际是**第四种情形**（预注册分支表的设计缺口，已如实登记）。 **★ 绝对强度退化**：vs 同一固定随机锚点 A **0.500±0.112** vs B **0.100±0.067**（Δ≈0.40≈5σ），B 三次触发 `<0.35` 警报，且 B vs `baseline_prev` 1.000（20/0）⇒ 「循环打转」指纹。 **跑中修正 1 处**（只影响未启动的 B：同目标重述不再重置 `_intent_age`；实测 `held` 2→16 帧）＋ **判读仪器补 1 项**（F2 的 40 帧塔伤/解围观察窗）。 | 判读 `docs/intent_ab_100k_judgment.md`、启动记录 `docs/intent_ab_100k_2026-09-19.md`、预注册 `docs/intent_save_prereg_2026-09-19.md`、日志 `docs/train_intent_off_100k.log` / `docs/train_intent_on_100k.log`、runner `scripts/run_intent_ab_100k.sh`、仪器 `scripts/audit_intent_save.py`（留证 `docs/audit_intent_save_B.txt` / `.json`）、台账 **O11**（`docs/agents/ledger.md` §6） |
| **`et_solo100k`**（`solo` 模式，**两臂** 各 100k；2026-09-18 **已跑完**） | **用户直接要求的「局面圣水交换」干预长跑（第八轮）**：`A_et = economy_et`（`engagement_trade=0.5` 同汇率）+ 阴性对照 `B_ctrl = economy_etm`（仅 `measure_only` 差），逐字同参同 seed 顺序跑，两臂各 **14/14** 评估点与回放、**0 降级**、`[gate] WARN` A 9 / B 6（只报警）。**判读（不构成判决）：没有任何一项能用现有仪器分辨出疗效** —— 机制层 4/6 **不可分辨**（余 3 项越界但绝对量极小 ⇒ 不写"改善/恶化"）；行为层差异停在**计数级**（全帧圣水≥6 **0.52% vs 0.05%**、Xbow **5/8769 vs 1/8370**、部署前圣水中位 3.250 vs 3.286）⇒ 两臂都**没**把"攒费→打 Xbow"做出来；门禁兑现率 **98.5% vs 98.2%**、逐批方向 +6/−7/0 ⇒ **饱和、无分辨力**（但 A 的"打出塔伤"低 **4.1 pp** 留作候选）。失败分支 **1/2/4 全部未触发**。缺阳性对照 + n=1/臂 ⇒ 只能写"观察到/不可分辨"。**六处测量侧更正**（§11.13.6/7/9/10/12/13，判据一字未改） | 判读 `docs/et_solo100k_judgment_2026-09-18.md`、读数 `docs/readout_et_solo100k.md`、预注册 `docs/engagement_trade_prereg_2026-09-18.md` §11.13、run 记录 `docs/et_solo100k_2026-09-18.md`、证据 `docs/readout_et_solo100k/`、台账 **O8**（`docs/agents/ledger.md` §6） |
...
| **★★ 在线口径实测：`p4b` 的优势不成立**（2026-09-18 第四轮）：`docs/online_measure_2026-09-18.md` —— measure-only 口径（`economy_etm` 预设；跑监视器、把逐帧局面写进录像 `et` 键、**不加一分奖励**）+ 250 局（5 评估点 × 50）+ 同批配对 ρ：**Δρ(塔血) +0.008 / Δρ(胜) +0.014**，对照离线口径的 +0.145/+0.108 ⇒ **÷18 / ÷7.7** ⇒ §11.8 的"配对 10/10 正"是**口径产物**；基准也翻号（`ρ(none,塔血)` 在线 +0.084 vs 离线 −0.198）⇒ **S2 门禁在正确口径下不通过**；`τ` 太稀疏（Στ/Σφ≈1/12、38% 对局 τ≠0）⇒ 换代理要换口径 | `scripts/analyze_online_trade.py`、`runs/et_measure/` |
| **★ 在线奖励项 + 落地验证 + 两处实质不确定**（2026-09-18 第三轮）：`docs/engagement_trade_online_2026-09-18.md` —— 在线监视器 `rl/engagement.py`（默认关）开/关逐位对账 PASS（`4d8b27b8…eca0c`）；记录通道 + schema 5 **已落真树**（中立性 DIGEST 一致 / selftest 4/4 / smoke 产出 schema 5：元组全 15 长、`v0` 100%、非塔 `root_cast` 97.9%）；★ 两处实质不确定：① 在线 vs 离线切分**不是同一个量**（同一局 67 vs 2 个窗口，`settle_frames` 1..20 无效）；② **重建 V 误差首次量出**：严格相等 44.9%、中位 0.23、p90 3.0、max 7.13（窗口 ΔΦ 量级 ±2）⇒ S3 仍不开 | `scripts/s2_trade_probe.py`、`scripts/_apply_s2_channel_when_idle.sh` |
