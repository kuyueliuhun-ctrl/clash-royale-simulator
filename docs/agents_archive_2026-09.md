# AGENTS 存档（2026-09-13 冻结）

> **这是什么**：2026-09-13 之前 `AGENTS.md` 的**存档头 + 原文正文**（**正文逐字未删改**，
> 保留全部过程叙述、证据链、逐点判读表与已过期的中间结论）。
>
> **怎么用**：
> - 日常决策与红线 → 读根目录 [`AGENTS.md`](../AGENTS.md)（精简版：红线 + 结论 + 索引）；
> - 需要**过程细节 / 原始证据 / 历史推理链**（"当时为什么这么判"）→ 来这里按主题搜；
> - 本文件是**冻结**的：新结论一律写进 `AGENTS.md` 与 `docs/`，**不要在这里追加**。
>
> 存档正文：100068 B / 1197 行（wc -l 口径），sha1=1ed210c56ab2（仅正文，用于核对未改动）。
