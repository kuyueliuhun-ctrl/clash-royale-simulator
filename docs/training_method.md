...
- **僵局早停（2026-09-17 起默认关闭）**：`cfg.train_stall_stop=False` 时**训练环与评估环都不再早停**，零塔损局打满到 180s/300s 由引擎按塔血合计裁决（用户拍板：早停的本意是防"**完全不下牌**"，**不是**防僵持 ⇒ 下牌但没打出伤害的对局按正常对待）。**端到端实测（`runs/nostall20k`，20k 步 / 评估节奏 4000）**：**J1–J3 全过** —— 回放逐帧复算 **0/240 局**被早停（对照 `demo20k` 32/280 = 11.4%，判据 = **全程零塔损**）、**0 平局**、`deploy%` 中位 **8.6→9.4** 且从未 <5% ⇒ **早停没有让模型更不爱下牌**；墙钟 **47 min**、训练侧局数 **62.7→58.0**（`demo20k` 47 局/15000 步 vs 本 run 58 局/20000 步）；终局罚可见度 `(γλ)^k` **0.27→0.011**（仍悬而未决）。完整判读 `docs/nostall20k_verdict_2026-09-17.md`（含两次自查更正：原估 "+96%~+222%" 与 "109→83~94" 均为**样本集混用**造成的错，已推翻）。开启时：每 `STALL_WINDOW = 10` 步探针一次，连续 `STALL_LIMIT = 10` 次零塔血变化 → 判平（常量与探针 `rl/run_league.py`，`--train-stall-stop` 复现旧行为）；早停结算用 `settle_stall(battle, cfg.stall_draw_margin)`（实现 `rl/run_league.py::settle_stall`）：**默认 `margin=0` ⇒ 与 `timeout_winner` 同口径**（三塔血量合计），仅 `margin>0` 才走 C′ 百分比+边距旧路径。 ⚠️**2026-09-17 口径变更**：见 `docs/draw_rule_verdict_2026-09-17.md`。
